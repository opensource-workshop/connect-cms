...
trait MigrationNc3Trait
{
    ...
    /**
     * 処理対象の判定
     * [TODO] common, インポート・エクスポート nc3, nc3共通
     */
    ...
    private function importSiteImpl($target, $target_plugin, $added, $redo = null)
    {
        ...
        if ($this->isTarget('cc_import', 'pages')) {
            ...
            foreach ($paths as $path) {
                ...
                // ルーム指定を探しておく。
                $room_id = null;
                if (array_key_exists('page_base', $page_ini) && array_key_exists('nc3_room_id', $page_ini['page_base'])) {
                    $room_id = $page_ini['page_base']['nc3_room_id'];
                }
    ...
    private function changePageInLink()
    {
        ...
        foreach ($contents as $content) {
            ...
            foreach ($hrefs as $href) {
                ...
                //    // NC3 ブロックID取得
                //    $nc3_block_id = mb_substr($href, mb_strripos($href, '#_') + 2);
                // #_が含まれているhrefを取得に変更 href="/hoge/fuga/#_123" に対応できないため
                if (preg_match('/#_(.*?)$/', $href, $m)) {
                    $nc3_block_id = $m[1];
                    // Connect-CMS フレームID
                    $map_frame = MigrationMapping::where('target_source_table', 'frames')->where('source_key', $nc3_block_id)->first();
                    if (!empty($map_frame)) {
                        $content->content_text = str_replace('#_' . $nc3_block_id, '#frame-' . $map_frame->destination_key, $content->content_text);
                        $content->save();
                    }
                }
            }
    ...
    private function changePageInLinkImpl($text)
    {
        ...
        foreach ($hrefs as $href) {
            ...
            if (mb_stripos($href, config('app.url')) === 0 || mb_stripos($href, '#_') === 0) {
                ...
                $map_frame = MigrationMapping::where('target_source_table', 'frames')->where('source_key', $nc3_block_id)->first();
                if (!empty($map_frame)) {
                    $text = str_replace('#_' . $nc3_block_id, '#frame-' . $map_frame->destination_key, $text);
                }
            }
        }
        return $text;
    ...
    private function importUploads($redo)
    {
        ...
        if (array_key_exists('uploads', $uploads_ini) && array_key_exists('upload', $uploads_ini['uploads'])) {
            foreach ($uploads_ini['uploads']['upload'] as $upload_key => $upload_item) {
                // ルーム指定を探しておく。
                $room_id = null;
                if (array_key_exists('nc3_room_id', $uploads_ini[$upload_key])) {
                    $room_id = $uploads_ini[$upload_key]['nc3_room_id'];
                }
    ...
    private function importUsers($redo)
    {
        ...
        // ユーザ任意項目の取り込み
        // ------------------------------------------
        // UsersColumns のコレクションを保持。後で入力データを移行する際に nc3_item_id でひっぱるため。
        ...
        foreach ($ini_paths as $ini_path) {
            ...
            if (in_array($column_type, ['radio','select','checkbox'])) {
                // マッピングテーブルの取得
                $mapping = MigrationMapping::where('target_source_table', 'users_columns_selects')->where('source_key', $nc3_item_id)->first();
                // マッピングテーブルを確認して、あれば削除
                if (!empty($mapping)) {
                    // ユーザセレクトカラム削除
        ...
        if (array_key_exists('users', $users_ini) && array_key_exists('user', $users_ini['users'])) {
            foreach ($users_ini['users']['user'] as $user_key => $username) {
                ...
                // パスワード変更
                $nc3_override_pass = $this->getMigrationConfig('users', 'nc3_export_login_users_overridepass');
                if (!empty($nc3_override_pass) && isset($nc3_override_pass[$user_item['userid']])) {
                    $user_item['password'] = $nc3_override_pass[$user_item['userid']];
                ...
                foreach ($create_users_columns as $create_users_column) {
                    $col = UsersInputCols::updateOrCreate([
                        ...
                        'value' => $user_item["item_{$create_users_column->nc3_item_id}"]
                    ]);
                }
    ...
    private function importGroups($redo)
    {
        ...
        foreach ($group_ini_paths as $i => $group_ini_path) {
            ...
            $source_page = MigrationMapping::where('target_source_table', 'nc3_pages')->where('source_key', $group_ini['source_info']['room_id'])->first();
        ...
        // BucketsMailの管理者グループ仮コード, 仮nc3ルームID置換
        ...
        if (array_key_exists('uploads', $uploads_ini) && array_key_exists('upload', $uploads_ini['uploads'])) {
            foreach ($uploads_ini['uploads']['upload'] as $upload_key => $upload_item) {
                ...
                $nc3_page = MigrationMapping::where('target_source_table', 'nc3_pages')->where('source_key', $room_id)->first();
                if (empty($nc3_page)) {
                    continue;
                }
                $connect_page = MigrationMapping::where('target_source_table', 'connect_page')->where('source_key', $nc3_page->destination_key)->first();
    ...
    /**
     * BucketsMailの管理者グループ仮コード, 仮nc3ルームIDからccグループID置換
     */
        ...
        foreach ($groups as &$group) {
            ...
            if (strpos($group, 'X-') === 0) {
                if ($group == 'X-管理者グループ') {
                    ...
                    // 仮nc3ルームID -> nc3ルームID
                    $nc3_room_id = str_ireplace('X-', '', $group);
                    // nc3ルームID -> グループID置換
                    $mapping = $groups_mappings->where('source_key', $nc3_room_id)->first();
    ...
    private function importBlogs($redo)
    {
        ...
        foreach ($blogs_ini_paths as $blogs_ini_path) {
            ...
            if (array_key_exists('source_info', $blog_ini) && array_key_exists('journal_id', $blog_ini['source_info'])) {
                $nc3_journal_id = $blog_ini['source_info']['journal_id'];
            }

            // ブログの統合
            $cc_import_marges = $this->getMigrationConfig('blogs', 'cc_import_marges');
            if (is_array($cc_import_marges) && array_key_exists($nc3_journal_id, $cc_import_marges)) {
                $nc3_journal_id = $cc_import_marges[$nc3_journal_id];
            }

            // マッピングテーブルの取得
            $mapping = MigrationMapping::where('target_source_table', 'blogs')->where('source_key', $nc3_journal_id)->first();
            ...
            // post_title のキーはNC3 の記事ID になっている。
            ...
            if (Storage::exists($this->getImportPath('blogs/') . $blog_tsv_filename)) {
                ...
                foreach ($blog_tsv_lines as $blog_tsv_line) {
                    ...
                    if ($blog_tsv_cols[9]) {
                        ...
                        // 移行はsession_id でも nc3 user_id でも、全てセッションIDに格納で処理
    ...
    private function importFaqs($redo)
    {
        ...
        foreach ($faqs_ini_paths as $faqs_ini_path) {
            ...
            if (array_key_exists('source_info', $faq_ini) && array_key_exists('faq_id', $faq_ini['source_info'])) {
                $nc3_faq_id = $faq_ini['source_info']['faq_id'];
            }

            // FAQの統合
            $cc_import_marges = $this->getMigrationConfig('faqs', 'cc_import_marges');
            if (is_array($cc_import_marges) && array_key_exists($nc3_faq_id, $cc_import_marges)) {
                $nc3_faq_id = $cc_import_marges[$nc3_faq_id];
            }

            // マッピングテーブルの取得
            $mapping = MigrationMapping::where('target_source_table', 'faqs')->where('source_key', $nc3_faq_id)->first();
            ...
            if (Storage::exists($this->getImportPath('faqs/') . $faq_tsv_filename)) {
                ...
                foreach ($faq_tsv_lines as $faq_tsv_line) {
                    ...
                    // 投稿日時の変換(NC3 の投稿日時はGMT のため、9時間プラスする) NC3=20151020122600
    ...
    private function importLinklists($redo)
    {
        ...
        foreach ($linklists_ini_paths as $linklists_ini_path) {
            ...
            if (array_key_exists('source_info', $linklist_ini) && array_key_exists('linklist_id', $linklist_ini['source_info'])) {
                $nc3_linklist_id = $linklist_ini['source_info']['linklist_id'];
            }

            // リンクリストの統合
            $cc_import_marges = $this->getMigrationConfig('linklists', 'cc_import_marges');
            if (is_array($cc_import_marges) && array_key_exists($nc3_linklist_id, $cc_import_marges)) {
                $nc3_linklist_id = $cc_import_marges[$nc3_linklist_id];
            }

            // マッピングテーブルの取得
            $mapping = MigrationMapping::where('target_source_table', 'linklists')->where('source_key', $nc3_linklist_id)->first();
    ...
    private function importDatabases($redo)
    {
        ...
        foreach ($databases_ini_paths as $databases_ini_path) {
            ...
            if (!empty($cc_import_where_database_ids)) {
                if (!in_array($nc3_multidatabase_id, $cc_import_where_database_ids)) {
                    continue;
                }
            }

            // マッピングテーブルの取得
            $mapping = MigrationMapping::where('target_source_table', 'databases')->where('source_key', $nc3_multidatabase_id)->first();
            ...
            if (empty($mapping)) {
                ...
                $mapping = MigrationMapping::create([
                    ...
                    'source_key'           => $nc3_multidatabase_id,
    ...
    private function importForms($redo)
    {
        ...
        foreach ($form_ini_paths as $form_ini_path) {
            ...
            // nc3 の registration_id
            $nc3_registration_id = 0;
            if (array_key_exists('source_info', $form_ini) && array_key_exists('registration_id', $form_ini['source_info'])) {
                $nc3_registration_id = $form_ini['source_info']['registration_id'];
            }

            // マッピングテーブルの取得
            $mapping = MigrationMapping::where('target_source_table', 'forms')->where('source_key', $nc3_registration_id)->first();
            ...
            $mapping = MigrationMapping::create([
                ...
                'source_key'           => $nc3_registration_id,
                'destination_key'      => $form->id,
            ]);

            // カラムID のNC3, Connect-CMS 変換テーブル（項目データの登録時に使うため）
            $column_ids = array();
    ...
    private function importWhatsnews($redo)
    {
        ...
        foreach ($whatsnew_ini_paths as $whatsnew_ini_paths) {
            ...
            // nc3 の whatsnew_block_id
            $nc3_whatsnew_block_id = 0;
            if (array_key_exists('source_info', $whatsnew_ini) && array_key_exists('whatsnew_block_id', $whatsnew_ini['source_info'])) {
                $nc3_whatsnew_block_id = $whatsnew_ini['source_info']['whatsnew_block_id'];
            }

            // マッピングテーブルの取得
            $mapping = MigrationMapping::where('target_source_table', 'whatsnews')->where('source_key', $nc3_whatsnew_block_id)->first();
            ...
            $mapping = MigrationMapping::create([
                ...
                'source_key'           => $nc3_whatsnew_block_id,
    ...
    private function importCabinets($redo)
    {
        ...
        foreach ($ini_paths as $ini_path) {
            ...
            if (array_key_exists('source_info', $ini) && array_key_exists('cabinet_id', $ini['source_info'])) {
                $nc3_cabinet_id = $ini['source_info']['cabinet_id'];
            }

            // マッピングテーブルの取得
            $mapping = MigrationMapping::where('target_source_table', 'cabinets')->where('source_key', $nc3_cabinet_id)->first();
    ...
    private function importBbses($redo)
    {
        ...
        foreach ($ini_paths as $ini_path) {
            ...
            // マッピングテーブルの取得
            $mapping = MigrationMapping::where('target_source_table', 'bbses')->where('source_key', $nc3_bbs_id)->first();
            ...
            $mapping = MigrationMapping::create([
                ...
                'source_key'           => $nc3_bbs_id,
                'destination_key'      => $bbs->id,
            ]);

            // 記事のマッピングテーブル作成用に記事一覧（post_title）を使用する。
            // post_title のキーはNC3 のBBSID になっている。
            $post_source_keys = array();
                    ...
                    if ($tsv_cols[13]) {
                        ...
                        for ($i = 0; $tsv_cols[13] > $i; $i++) {
                            $like_users = LikeUser::create([
                                ...
                                'target_contents_id' => $bbs_post->id,
                                'likes_id' => $like->id,
                                'session_id' => null,   // nc3bbsは session_id を保持していないため、nullセット
                                'users_id' => null,
                            ]);
                        }
                    }
    ...
    private function importCounters($redo)
    {
        ...
        foreach ($ini_paths as $ini_path) {
            ...
            // nc3 の counter_block_id
            $nc3_counter_block_id = 0;
            if (array_key_exists('source_info', $ini) && array_key_exists('counter_block_id', $ini['source_info'])) {
                $nc3_counter_block_id = $ini['source_info']['counter_block_id'];
            }

            // マッピングテーブルの取得
            $mapping = MigrationMapping::where('target_source_table', 'counters')->where('source_key', $nc3_counter_block_id)->first();
            ...
            $mapping = MigrationMapping::create([
                ...
                'source_key'           => $nc3_counter_block_id,
    ...
    private function importCalendars($redo)
    {
        ...
        foreach ($ini_paths as $ini_path) {
            ...
            // nc3 の calendar_room_id
            $nc3_calendar_room_id = 0;
            if (array_key_exists('source_info', $ini) && array_key_exists('room_id', $ini['source_info'])) {
                $nc3_calendar_room_id = $ini['source_info']['room_id'];
            }

            // マッピングテーブルの取得
            $mapping = MigrationMapping::where('target_source_table', 'calendars')->where('source_key', $nc3_calendar_room_id)->first();
            ...
            if (CalendarPost::where('calendar_id', $calendar->id)->count() == 0) {
                ...
                $mapping = MigrationMapping::create([
                    ...
                    'source_key'           => $nc3_calendar_room_id,
    ...
    private function importSlideshows($redo)
    {
        ...
        foreach ($ini_paths as $ini_path) {
            ...
            // nc3 の block_id
            $nc3_slideshows_block_id = 0;
            if (array_key_exists('source_info', $ini) && array_key_exists('slideshows_block_id', $ini['source_info'])) {
                $nc3_slideshows_block_id = $ini['source_info']['slideshows_block_id'];
            }

            // マッピングテーブルの取得
            $mapping = MigrationMapping::where('target_source_table', 'slideshows')->where('source_key', $nc3_slideshows_block_id)->first();
            ...
            if (empty($mapping)) {
                ...
                $mapping = MigrationMapping::create([
                    ...
                    'source_key'           => $nc3_slideshows_block_id,
    ...
    private function importSimplemovie($redo)
    {
        ...
        foreach ($ini_paths as $ini_path) {
            ...
            // nc3 の block_id
            $nc3_simplemovie_block_id = 0;
            if (array_key_exists('source_info', $ini) && array_key_exists('simplemovie_block_id', $ini['source_info'])) {
                $nc3_simplemovie_block_id = $ini['source_info']['simplemovie_block_id'];
            }

            // マッピングテーブルの取得
            $mapping = MigrationMapping::where('target_source_table', 'simplemovie')->where('source_key', $nc3_simplemovie_block_id)->first();
            ...
            $mapping = MigrationMapping::create([
                ...
                'source_key'           => $nc3_simplemovie_block_id,
    ...
    private function importReservations($redo)
    {
        ...
        // 施設カテゴリの取り込み
        // ------------------------------------------
        // ReservationsCategory のコレクションを保持。後で入力データを移行する際に nc3_category_id でひっぱるため。
        ...
        foreach ($ini_paths as $ini_path) {
            ...
            if (array_key_exists('source_info', $ini) && array_key_exists('category_id', $ini['source_info'])) {
                $nc3_category_id = $ini['source_info']['category_id'];
            }

            // マッピングテーブルの取得
            $mapping = MigrationMapping::where('target_source_table', 'reservations_category')->where('source_key', $nc3_category_id)->first();
            ...
            $reservation_categories->nc3_category_id = $ini['source_info']['category_id'];
            // コレクションに要素追加
        ...
        foreach ($ini_paths as $ini_path) {
            ...
            // nc3 の nc3_location_id
            $nc3_location_id = 0;
            if (array_key_exists('source_info', $ini) && array_key_exists('location_id', $ini['source_info'])) {
                $nc3_location_id = $ini['source_info']['location_id'];
            }

            // マッピングテーブルの取得
            $mapping = MigrationMapping::where('target_source_table', 'reservations_location')->where('source_key', $nc3_location_id)->first();
            ...
            // 対象カテゴリ
            $create_reservation_category = $create_reservation_categories->firstWhere('nc3_category_id', $ini['reservation_location']['category_id']);
            ...
            // １つ前の予約ID
            $before_nc3_reserve_details_id = null;
            // １つ前の親ID
            ...
            if (Storage::exists($this->getImportPath('reservations/') . $reservation_tsv_filename)) {
                ...
                foreach ($reservation_tsv_lines as $reservation_tsv_line) {
                    ...
                    if ($before_nc3_reserve_details_id != $reservation_tsv_cols[$tsv_idxs['reserve_details_id']]) {
                        ...
                        if ($rrule_setting) {
                            ...
                            // 開始日
                            // ※ [要注意] NC3の reservation_reserve_details.rrule は DTSTART がないため、移行時は開始日の補完が必要。
                    ...
                    $before_nc3_reserve_details_id = $reservation_tsv_cols[$tsv_idxs['reserve_details_id']];
        ...
        foreach ($ini_paths as $ini_path) {
            ...
            // nc3 の reservation_block_id
            $nc3_reservation_block_id = 0;
            if (array_key_exists('source_info', $ini) && array_key_exists('reservation_block_id', $ini['source_info'])) {
                $nc3_reservation_block_id = $ini['source_info']['reservation_block_id'];
            }

            // マッピングテーブルの取得
            $mapping = MigrationMapping::where('target_source_table', 'reservations_block')->where('source_key', $nc3_reservation_block_id)->first();
            ...
            $mapping = MigrationMapping::create([
                ...
                'source_key'           => $nc3_reservation_block_id,
    ...
    private function importPhotoalbums($redo)
    {
        ...
        foreach ($photoalbums_ini_paths as $photoalbums_ini_path) {
            ...
            if (!empty($cc_import_where_photoalbum_ids)) {
                if (!in_array($nc3_photoalbum_id, $cc_import_where_photoalbum_ids)) {
                    continue;
                }
            }

            // マッピングテーブルの取得
            $mapping = MigrationMapping::where('target_source_table', 'photoalbums')->where('source_key', $nc3_photoalbum_id)->first();
            ...
            foreach ($this->getArrayValue($photoalbums_ini, 'albums', 'album', []) as $album_id => $album_name) {
                ...
                if (empty($mapping_album)) {
                    ...
                    if ($photoalbums_ini[$album_id]['nc3_upload_id']) {
                        // アルバムのジャケット登録
                        $contents = [
                            'photoalbum_id' => $photoalbum->id,
                            'nc3_upload_id' => $photoalbums_ini[$album_id]['nc3_upload_id'],
                            'name'          => $album_name,
                        ...
                        $mapping_album_cover_tmp = MigrationMapping::create([
                            ...
                            'source_key'           => $photoalbums_ini[$album_id]['nc3_upload_id'],
                    ...
                    $mapping_album_cover = MigrationMapping::where('target_source_table', 'photoalbums_album_cover')
                        ->where('source_key', $photoalbums_ini[$album_id]['nc3_upload_id'])
                        ->first();
                ...
                $photoalbums_tsv_path = $this->getImportPath('photoalbums/photoalbum_') . $this->zeroSuppress($nc3_photoalbum_id) . '_' . $this->zeroSuppress($album_id) . '.tsv';
                ...
                if (Storage::exists($photoalbums_tsv_path)) {
                    ...
                    foreach ($photoalbum_tsv_lines as $photoalbum_tsv_line) {
                        ...
                        if (empty($mapping_photo)) {
                            ...
                            $contents = [
                                'photoalbum_id' => $photoalbum->id,
                                'nc3_upload_id' => $photoalbum_tsv_cols[$tsv_idxs['nc3_upload_id']],
                                ...
                                'is_cover' => PhotoalbumContent::is_cover_off,
                                // 'nc3_photo_id' => $photoalbum_tsv_cols[$tsv_idxs['photo_id']],
    ...
    {
        $upload_mapping = $upload_mappings->firstWhere('source_key', $contents['nc3_upload_id']);
        $upload = null;
        if ($upload_mapping) {
            $upload = $uploads_all->firstWhere('id', $upload_mapping->destination_key);
            if (!$upload) {
                $this->putMonitor(3, "Connectの Uploads にアップロードIDなし。nc3_album_name={$contents['name']}, nc3_upload_id={$contents['nc3_upload_id']}, is_cover={$contents['is_cover']}");
            }
        } else {
            $this->putMonitor(3, "Connectの MigrationMapping にアップロードIDなし。nc3_album_name={$contents['name']}, nc3_upload_id={$contents['nc3_upload_id']}, is_cover={$contents['is_cover']}\n");
        }
    ...
    private function changeWYSIWYG($content)
    {
        ...
        foreach ($change_list as $image_path) {
            if (strpos($image_path, '../../uploads') === 0) {
                ...
                $nc3_upload_id = array_search($img_filename, $this->uploads_ini['uploads']['upload']);
                $upload_mapping = MigrationMapping::where('target_source_table', 'uploads')->where('source_key', $nc3_upload_id)->first();
                if (!empty($upload_mapping)) {
                    $content = str_replace($image_path, $this->getImportSrcDir() . $upload_mapping->destination_key, $content);
                } else {
                    // $this->putError(1, 'image path not found mapping', "コンテンツ中のアップロード画像のパスがマッピングテーブルに見つからない。nc3_upload_id = " . $nc3_upload_id);
                }
            }
        }
        return $content;
    ...
    private function parseIni($ini_path)
    {
        ...
        //    return $ini;
        //}
        //$nc3_block_overwrite_path = 'migration/@addition/nc3_blocks/block_overwrite_' . $source_key . '.ini';
        //if (Storage::exists($nc3_block_overwrite_path)) {
        //    $overwrite_ini = parse_ini_file(storage_path() . '/app/' . $nc3_block_overwrite_path, true);
    ...
    private function importPluginMenus($page, $page_dir, $frame_ini, $display_sequence)
    {
        ...
        if (!empty($ommit_page_ids_nc3)) {
            foreach (explode(",", $ommit_page_ids_nc3) as $ommit_page_id_nc3) {
                $nc3_page = MigrationMapping::where('target_source_table', 'nc3_pages')
                                            ->where('source_key', $ommit_page_id_nc3)
                                            ->first();
                if (!empty($nc3_page)) {
                    $connect_page = MigrationMapping::where('target_source_table', 'connect_page')
                                                    ->where('source_key', $nc3_page->destination_key)
                                                    ->first();
                }
                if (!empty($connect_page)) {
    ...
    private function importPluginBlogs($page, $page_dir, $frame_ini, $display_sequence)
    {
        ...
        if (!empty($blogs)) {
            ...
            if (array_key_exists('blog_base', $blog_ini) && array_key_exists('view_count', $blog_ini['blog_base'])) {
                ...
                // view_count が 0 を含む空の場合は、初期値にする。（NC3 で0 で全件表示されているものがあるので、その対応）
                if (empty($view_count)) {
                    $view_count = 10;
                }
            }
    ...
    private function importPluginFaqs($page, $page_dir, $frame_ini, $display_sequence)
    {
        ...
        // NC3 のfaq_id でマップ確認
        if (!empty($faq_ini) && array_key_exists('source_info', $faq_ini) && array_key_exists('faq_id', $faq_ini['source_info'])) {
            $migration_mappings = MigrationMapping::where('target_source_table', 'faqs')->where('source_key', $nc3_faq_id)->first();
        }
        // マップから新Blog を取得
        if (!empty($migration_mappings)) {
    ...
    private function importPluginLinklists($page, $page_dir, $frame_ini, $display_sequence)
    {
        ...
        // NC3 のlinklist_id でマップ確認
        if (!empty($linklist_ini) && array_key_exists('source_info', $linklist_ini) && array_key_exists('linklist_id', $linklist_ini['source_info'])) {
            $migration_mappings = MigrationMapping::where('target_source_table', 'linklists')->where('source_key', $nc3_linklist_id)->first();
        }
        // マップから新リンクリストを取得
        if (!empty($migration_mappings)) {
    ...
    private function importPluginCabinets($page, $page_dir, $frame_ini, $display_sequence)
    {
        ...
        if (!empty($cabinet_ini) && array_key_exists('source_info', $cabinet_ini) && array_key_exists('cabinet_id', $cabinet_ini['source_info'])) {
            $nc3_cabinet_id = $cabinet_ini['source_info']['cabinet_id'];
        }
        // NC3 のcabinet_id でマップ確認
        if (!empty($cabinet_ini) && array_key_exists('source_info', $cabinet_ini) && array_key_exists('cabinet_id', $cabinet_ini['source_info'])) {
            $migration_mapping = MigrationMapping::where('target_source_table', 'cabinets')->where('source_key', $nc3_cabinet_id)->first();
        }
        // マップから新Cabinet を取得
    ...
    private function importPluginBbses($page, $page_dir, $frame_ini, $display_sequence)
    {
        ...
        // NC3 のbbs_id でマップ確認
        if (!empty($bbs_ini) && array_key_exists('source_info', $bbs_ini) && array_key_exists('journal_id', $bbs_ini['source_info'])) {
            $migration_mapping = MigrationMapping::where('target_source_table', 'bbses')->where('source_key', $nc3_bbs_id)->first();
        }
        // マップから新bbs を取得
        if (!empty($migration_mapping)) {
        ...
        if (!empty($bucket)) {
            // 権限設定
            // ---------------------------------------
            // 投稿権限：(nc3) 掲示板単位であり、(cc) バケツ単位であり
            // 承認権限：(nc3) なし、(cc) あり => buckets_roles.approval_flag = 0固定
            BucketsRoles::updateOrCreate(
                [
                    'buckets_id' => $bucket->id,
                    'role' => 'role_article',   // モデレータ
    ...
    private function importPluginWhatsnews($page, $page_dir, $frame_ini, $display_sequence)
    {
        ...
        // フレームのエクスポートファイルから、NC3 の新着情報のID 取得
        if (array_key_exists('frame_base', $frame_ini) && array_key_exists('whatsnew_block_id', $frame_ini['frame_base'])) {
            $nc3_whatsnew_block_id = $frame_ini['frame_base']['whatsnew_block_id'];
        }
        // エクスポートした新着情報の設定内容の取得
        if (!empty($nc3_whatsnew_block_id) && Storage::exists($this->getImportPath('whatsnews/whatsnew_') . $nc3_whatsnew_block_id . '.ini')) {
            $whatsnew_ini = parse_ini_file(storage_path() . '/app/' . $this->getImportPath('whatsnews/whatsnew_') . $nc3_whatsnew_block_id . '.ini', true);
        }
        // NC3 の whatsnew_block_id でマップ確認
    ...
    private function importPluginCounters($page, $page_dir, $frame_ini, $display_sequence)
    {
        ...
        if (!empty($counter_ini) && array_key_exists('source_info', $counter_ini) && array_key_exists('counter_block_id', $counter_ini['source_info'])) {
            $migration_mapping = MigrationMapping::where('target_source_table', 'counters')->where('source_key', $nc3_counter_block_id)->first();
        }
        // マップから新Counter を取得
        if (!empty($migration_mapping)) {
    ...
    private function importPluginCalendars($page, $page_dir, $frame_ini, $display_sequence)
    {
        ...
        // カレンダールームの情報取得
        if (!empty($nc3_calendar_room_id) && Storage::exists($this->getImportPath('calendars/calendar_room_') . $this->zeroSuppress($nc3_calendar_room_id) . '.ini')) {
            // dd($nc3_calendar_room_id);

            $calendar_room_ini = parse_ini_file(storage_path() . '/app/' . $this->getImportPath('calendars/calendar_room_') . $this->zeroSuppress($nc3_calendar_room_id) . '.ini', true);
        }
        ...
        if (!empty($bucket)) {
            ...
            if (!empty($calendar_room_ini) && array_key_exists('calendar_manage', $calendar_room_ini) && array_key_exists('add_authority_id', $calendar_room_ini['calendar_manage'])) {
                ...
                //   (nc3) モデレータ⇒ (cc) モデレータ
                //   (nc3) 一般⇒ (cc) 編集者
                //   (nc3) [calendar_manage] => add_authority_id, 予定を追加できる権限. 2:主担,モデレータ,一般  3:主担,モデレータ  4:主担  5:なし（全会員のみ設定可能）
                // 承認権限：(nc3) なし、(cc) あり => buckets_roles.approval_flag = 0固定

                // モデレータの投稿権限 変換 (key:nc3)add_authority_id => (value:cc)post_flag
                ...
                // 編集者の投稿権限 変換 (key:nc3)add_authority_id => (value:cc)post_flag
    ...
    private function importPluginSlideshows($page, $page_dir, $frame_ini, $display_sequence)
    {
        ...
        if (!empty($slideshow_ini) && array_key_exists('source_info', $slideshow_ini) && array_key_exists('slideshows_block_id', $slideshow_ini['source_info'])) {
            $migration_mapping = MigrationMapping::where('target_source_table', 'slideshows')->where('source_key', $nc3_slideshows_block_id)->first();
        }
    ...
    private function importPluginSimplemovie($page, $page_dir, $frame_ini, $display_sequence)
    {
        ...
        if (!empty($simplemovie_ini) && array_key_exists('source_info', $simplemovie_ini) && array_key_exists('simplemovie_block_id', $simplemovie_ini['source_info'])) {
            $migration_mapping = MigrationMapping::where('target_source_table', 'simplemovie')->where('source_key', $nc3_simplemovie_block_id)->first();
        }
/* TODO ここがおかしいはず */
    ...
    private function importPluginReservations($page, $page_dir, $frame_ini, $display_sequence)
    {
        ...
        if (!empty($bucket)) {
            ...
            // 権限設定
            // ---------------------------------------
            // 投稿権限：(nc3) 施設単位であり、(cc) バケツ単位であり =>  buckets_roles.post_flag = 1固定
            // 承認権限：(nc3) なし、(cc) あり => buckets_roles.approval_flag = 0固定
            BucketsRoles::updateOrCreate(
                [
                    'buckets_id' => $bucket->id,
    ...
    private function importPluginContents($page, $page_dir, $frame_ini, $display_sequence)
    {
        ...
        // NC3 から移行した場合：[upload_images] の画像を登録
        //
        // --- uploads.ini
        ...
        if (array_key_exists('upload_images', $frame_ini) && array_key_exists('upload', $frame_ini['upload_images'])) {
            // アップロードファイル定義のループ
            foreach ($frame_ini['upload_images']['upload'] as $nc3_upload_id => $image_path) {
                // 画像のパスの修正
                // ini ファイルのID はNC3 のアップロードID が入っている。
                // マッピングテーブルから新ID を取得して、変換する。
                $migration_mapping = MigrationMapping::where('target_source_table', 'uploads')->where('source_key', $nc3_upload_id)->first();
        ...
        // NC3 から移行した場合：[upload_files] のファイルを登録
        if (array_key_exists('upload_files', $frame_ini) && array_key_exists('upload', $frame_ini['upload_files'])) {
            foreach ($frame_ini['upload_files']['upload'] as $nc3_upload_id => $file_path) {
                // アップロードファイルのパスの修正
                // ini ファイルのID はNC3 のアップロードID が入っている。
                // マッピングテーブルから新ID を取得して、変換する。
                $migration_mapping = MigrationMapping::where('target_source_table', 'uploads')->where('source_key', $nc3_upload_id)->first();
                ...
                if (!empty($migration_mapping)) {
                    ...
                    // $this->putError(1, 'file path not found mapping', "コンテンツ中のアップロードファイルのパスがマッピングテーブルに見つからない。nc3_upload_id = " . $nc3_upload_id);
                }
            }
        }
     ...
     */
    private function changeFullPath($content, $nc3_page)
    {
            ...
            if ($match_count !== false) {
                for ($i = 0; $i < $match_count; $i++) {
                    ...
                    if (stripos($matches[1][$i], './') === 0) {
                        $replace_str = str_ireplace('./', '/' . $nc3_page->permalink . '/', $matches[0][$i]);
                        $content = str_ireplace($matches[0][$i], $replace_str, $content);
                    }
                    // 何階層上がるか計算
                    $count = mb_substr_count($matches[1][$i], '../');
                    if ($count > 0) {
                        $new_permalink = $nc3_page->permalink;
                        // 上がる階層分、右から / の左まで切り取り
                        for ($j = 0; $j < $count; $j++) {
...
//////////////////
        ...
        if ($this->isTarget('nc3_export', 'pages')) {
            ...
            foreach ($nc3_sort_pages as $nc3_sort_page_key => $nc3_sort_page) {
                ...
                // echo $nc3_sort_page_key . ':' . $nc3_sort_page->page_name . "\n";
    ...
    private function nc3ExportJournal($redo)
    {
        ...
        foreach ($nc3_journals as $nc3_journal) {
            ...
            $journals_ini .= "created_at      = \"" . $this->getCCDatetime($nc3_journal->insert_time) . "\"\n";
            $journals_ini .= "created_name    = \"" . $nc3_journal->insert_user_name . "\"\n";
            $journals_ini .= "insert_login_id = \"" . $this->getNc2LoginIdFromNc2UserId($nc3_users, $nc3_journal->insert_user_id) . "\"\n";
            $journals_ini .= "updated_at      = \"" . $this->getCCDatetime($nc3_journal->update_time) . "\"\n";
            $journals_ini .= "updated_name    = \"" . $nc3_journal->update_user_name . "\"\n";
            $journals_ini .= "update_login_id = \"" . $this->getNc2LoginIdFromNc2UserId($nc3_users, $nc3_journal->update_user_id) . "\"\n";
            ...
            foreach ($nc3_journal_posts as $nc3_journal_post) {
                ...
                $journals_tsv .= $this->getCCDatetime($nc3_journal_post->insert_time)                             . "\t";   // [11]
                $journals_tsv .= $nc3_journal_post->insert_user_name                                              . "\t";   // [12]
                $journals_tsv .= $this->getNc2LoginIdFromNc2UserId($nc3_users, $nc3_journal_post->insert_user_id) . "\t";   // [13]
                $journals_tsv .= $this->getCCDatetime($nc3_journal_post->update_time)                             . "\t";   // [14]
                $journals_tsv .= $nc3_journal_post->update_user_name                                              . "\t";   // [15]
                $journals_tsv .= $this->getNc2LoginIdFromNc2UserId($nc3_users, $nc3_journal_post->update_user_id);          // [16]
    ...
    private function nc3ExportBbs($redo)
    {
        ...
        foreach ($nc3_bbses as $nc3_bbs) {
            ...
            $journals_ini .= "created_at      = \"" . $this->getCCDatetime($nc3_bbs->insert_time) . "\"\n";
            $journals_ini .= "created_name    = \"" . $nc3_bbs->insert_user_name . "\"\n";
            $journals_ini .= "insert_login_id = \"" . $this->getNc2LoginIdFromNc2UserId($nc3_users, $nc3_bbs->insert_user_id) . "\"\n";
            $journals_ini .= "updated_at      = \"" . $this->getCCDatetime($nc3_bbs->update_time) . "\"\n";
            $journals_ini .= "updated_name    = \"" . $nc3_bbs->update_user_name . "\"\n";
            $journals_ini .= "update_login_id = \"" . $this->getNc2LoginIdFromNc2UserId($nc3_users, $nc3_bbs->update_user_id) . "\"\n";
            ...
            foreach ($nc3_bbs_posts as $nc3_bbs_post) {
                ...
                $journals_tsv .= $this->getCCDatetime($nc3_bbs_post->insert_time) . "\t"; // 0:投稿日時
                $journals_tsv .=                              "\t"; // カテゴリ
                ...
                $journals_tsv .= $this->getNc2LoginIdFromNc2UserId($nc3_users, $nc3_bbs_post->insert_user_id) . "\t";   // 15:投稿者ID
                $journals_tsv .= $this->getCCDatetime($nc3_bbs_post->update_time) . "\t";                               // 16:更新日時
                $journals_tsv .= $nc3_bbs_post->update_user_name . "\t";                                                // 17:更新者名
                $journals_tsv .= $this->getNc2LoginIdFromNc2UserId($nc3_users, $nc3_bbs_post->update_user_id) . "\t";   // 18:更新者ID
    ...
    private function nc3ExportFaq($redo)
    {
        ...
        foreach ($nc3_faqs as $nc3_faq) {
            ...
            foreach ($nc3_faq_questions as $nc3_faq_question) {
                ...
                $faqs_tsv .= $nc3_faq_question->insert_time      . "\t";
    ...
    private function nc3ExportLinklist($redo)
    {
        ...
        foreach ($nc3_linklists as $nc3_linklist) {
            ...
            $linklists_ini .= "created_at      = \"" . $this->getCCDatetime($nc3_linklist->insert_time) . "\"\n";
            $linklists_ini .= "created_name    = \"" . $nc3_linklist->insert_user_name . "\"\n";
            $linklists_ini .= "insert_login_id = \"" . $this->getNc2LoginIdFromNc2UserId($nc3_users, $nc3_linklist->insert_user_id) . "\"\n";
            $linklists_ini .= "updated_at      = \"" . $this->getCCDatetime($nc3_linklist->update_time) . "\"\n";
            $linklists_ini .= "updated_name    = \"" . $nc3_linklist->update_user_name . "\"\n";
            $linklists_ini .= "update_login_id = \"" . $this->getNc2LoginIdFromNc2UserId($nc3_users, $nc3_linklist->update_user_id) . "\"\n";
    ...
    private function nc3ExportMultidatabase($redo)
    {
        ...
        foreach ($nc3_multidatabases as $nc3_multidatabase) {
            ...
            $multidatabase_ini .= "created_at      = \"" . $this->getCCDatetime($nc3_multidatabase->insert_time) . "\"\n";
            $multidatabase_ini .= "created_name    = \"" . $nc3_multidatabase->insert_user_name . "\"\n";
            $multidatabase_ini .= "insert_login_id = \"" . $this->getNc2LoginIdFromNc2UserId($nc3_users, $nc3_multidatabase->insert_user_id) . "\"\n";
            $multidatabase_ini .= "updated_at      = \"" . $this->getCCDatetime($nc3_multidatabase->update_time) . "\"\n";
            $multidatabase_ini .= "updated_name    = \"" . $nc3_multidatabase->update_user_name . "\"\n";
            $multidatabase_ini .= "update_login_id = \"" . $this->getNc2LoginIdFromNc2UserId($nc3_users, $nc3_multidatabase->update_user_id) . "\"\n";
            ...
            foreach ($multidatabase_metadata_contents as $multidatabase_metadata_content) {
                ...
                if ($content_id != $multidatabase_metadata_content->content_id) {
                    if ($content_id == 0) {
                        ...
                        $tsv_record['insert_login_id'] = $this->getNc2LoginIdFromNc2UserId($nc3_users, $old_metadata_content->multidatabase_content_insert_user_id);
                        ...
                        $tsv_record['update_login_id'] = $this->getNc2LoginIdFromNc2UserId($nc3_users, $old_metadata_content->multidatabase_content_update_user_id);
                        // NC3 レコードを示すID
    ...
    private function nc3ExportRegistration($redo)
    {
        ...
        foreach ($nc3_registrations as $nc3_registration) {
            ...
            $registration_ini .= "created_at      = \"" . $this->getCCDatetime($nc3_registration->insert_time) . "\"\n";
            $registration_ini .= "created_name    = \"" . $nc3_registration->insert_user_name . "\"\n";
            $registration_ini .= "insert_login_id = \"" . $this->getNc2LoginIdFromNc2UserId($nc3_users, $nc3_registration->insert_user_id) . "\"\n";
            $registration_ini .= "updated_at      = \"" . $this->getCCDatetime($nc3_registration->update_time) . "\"\n";
            $registration_ini .= "updated_name    = \"" . $nc3_registration->update_user_name . "\"\n";
            $registration_ini .= "update_login_id = \"" . $this->getNc2LoginIdFromNc2UserId($nc3_users, $nc3_registration->update_user_id) . "\"\n";
            ...
            if ($this->hasMigrationConfig('forms', 'nc3_export_registration_data', true)) {
                ...
                foreach ($registration_item_datas as $registration_item_data) {
                    if ($registration_item_data->data_id != $data_id) {
                        ...
                        $registration_data .= "insert_login_id = \"" . $this->getNc2LoginIdFromNc2UserId($nc3_users, $registration_item_data->data_insert_user_id) . "\"\n";
                        ...
                        $registration_data .= "update_login_id = \"" . $this->getNc2LoginIdFromNc2UserId($nc3_users, $registration_item_data->data_update_user_id) . "\"\n";
    ...
    private function nc3ExportWhatsnew($redo)
    {
        ...
        foreach ($nc3_whatsnew_blocks as $nc3_whatsnew_block) {
            ...
            $whatsnew_ini .= "created_at      = \"" . $this->getCCDatetime($nc3_whatsnew_block->insert_time) . "\"\n";
            $whatsnew_ini .= "created_name    = \"" . $nc3_whatsnew_block->insert_user_name . "\"\n";
            $whatsnew_ini .= "insert_login_id = \"" . $this->getNc2LoginIdFromNc2UserId($nc3_users, $nc3_whatsnew_block->insert_user_id) . "\"\n";
            $whatsnew_ini .= "updated_at      = \"" . $this->getCCDatetime($nc3_whatsnew_block->update_time) . "\"\n";
            $whatsnew_ini .= "updated_name    = \"" . $nc3_whatsnew_block->update_user_name . "\"\n";
            $whatsnew_ini .= "update_login_id = \"" . $this->getNc2LoginIdFromNc2UserId($nc3_users, $nc3_whatsnew_block->update_user_id) . "\"\n";
    ...
    private function nc3ExportCabinet($redo)
    {
        ...
        foreach ($cabinet_manages as $cabinet_manage) {
            ...
            $ini .= "insert_login_id = \"" . $this->getNc2LoginIdFromNc2UserId($nc3_users, $cabinet_manage->insert_user_id) . "\"\n";
            $ini .= "updated_at      = \"" . $this->getCCDatetime($cabinet_manage->update_time) . "\"\n";
            $ini .= "updated_name    = \"" . $cabinet_manage->update_user_name . "\"\n";
            $ini .= "update_login_id = \"" . $this->getNc2LoginIdFromNc2UserId($nc3_users, $cabinet_manage->update_user_id) . "\"\n";
            ...
            foreach ($cabinet_files as $index => $cabinet_file) {
                ...
                $tsv .= $this->getNc2LoginIdFromNc2UserId($nc3_users, $cabinet_file->insert_user_id) . "\t";    // [15]
                $tsv .= $this->getCCDatetime($cabinet_file->update_time)                             . "\t";    // [16]
                $tsv .= $cabinet_file->update_user_name                                              . "\t";    // [17]
                $tsv .= $this->getNc2LoginIdFromNc2UserId($nc3_users, $cabinet_file->update_user_id);           // [18]
    ...
    private function nc3ExportCounter($redo)
    {
        ...
        foreach ($nc3_counters as $nc3_counter) {
            ...
            $ini .= "created_at      = \"" . $this->getCCDatetime($nc3_counter->insert_time) . "\"\n";
            $ini .= "created_name    = \"" . $nc3_counter->insert_user_name . "\"\n";
            $ini .= "insert_login_id = \"" . $this->getNc2LoginIdFromNc2UserId($nc3_users, $nc3_counter->insert_user_id) . "\"\n";
            $ini .= "updated_at      = \"" . $this->getCCDatetime($nc3_counter->update_time) . "\"\n";
            $ini .= "updated_name    = \"" . $nc3_counter->update_user_name . "\"\n";
            $ini .= "update_login_id = \"" . $this->getNc2LoginIdFromNc2UserId($nc3_users, $nc3_counter->update_user_id) . "\"\n";
    ...
    private function nc3ExportCalendar($redo)
    {
        ...
        foreach ($nc3_page_rooms as $nc3_page_room) {
            ...
            foreach ($calendar_plans as $calendar_plan) {
                ...
                $tsv_record['insert_login_id'] = $this->getNc2LoginIdFromNc2UserId($nc3_users, $calendar_plan->insert_user_id);
                $tsv_record['updated_at']      = $this->getCCDatetime($calendar_plan->update_time);
                $tsv_record['updated_name']    = $calendar_plan->update_user_name;
                $tsv_record['update_login_id'] = $this->getNc2LoginIdFromNc2UserId($nc3_users, $calendar_plan->update_user_id);
    ...
    private function nc3ExportSlides($redo)
    {
        ...
        foreach ($nc3_slideshows as $nc3_slideshow) {
            ...
            $ini .= "created_at      = \"" . $this->getCCDatetime($nc3_slideshow->insert_time) . "\"\n";
            $ini .= "created_name    = \"" . $nc3_slideshow->insert_user_name . "\"\n";
            $ini .= "insert_login_id = \"" . $this->getNc2LoginIdFromNc2UserId($nc3_users, $nc3_slideshow->insert_user_id) . "\"\n";
            $ini .= "updated_at      = \"" . $this->getCCDatetime($nc3_slideshow->update_time) . "\"\n";
            $ini .= "updated_name    = \"" . $nc3_slideshow->update_user_name . "\"\n";
            $ini .= "update_login_id = \"" . $this->getNc2LoginIdFromNc2UserId($nc3_users, $nc3_slideshow->update_user_id) . "\"\n";
    ...
    private function nc3ExportReservation($redo)
    {
        ...
        foreach ($nc3_reservation_locations as $nc3_reservation_location) {
            ...
            foreach ($reservation_reserves as $reservation_reserve) {
                ...
                $tsv_record['insert_login_id'] = $this->getNc2LoginIdFromNc2UserId($nc3_users, $reservation_reserve->insert_user_id);
                ...
                $tsv_record['update_login_id'] = $this->getNc2LoginIdFromNc2UserId($nc3_users, $reservation_reserve->update_user_id);
    ...
    private function nc3ExportPhotoalbum($redo)
    {
        ...
        foreach ($nc3_photoalbums as $nc3_photoalbum) {
            ...
            $photoalbum_ini .= "created_at      = \"" . $this->getCCDatetime($nc3_photoalbum->insert_time) . "\"\n";
            $photoalbum_ini .= "created_name    = \"" . $nc3_photoalbum->insert_user_name . "\"\n";
            $photoalbum_ini .= "insert_login_id = \"" . $this->getNc2LoginIdFromNc2UserId($nc3_users, $nc3_photoalbum->insert_user_id) . "\"\n";
            $photoalbum_ini .= "updated_at      = \"" . $this->getCCDatetime($nc3_photoalbum->update_time) . "\"\n";
            $photoalbum_ini .= "updated_name    = \"" . $nc3_photoalbum->update_user_name . "\"\n";
            $photoalbum_ini .= "update_login_id = \"" . $this->getNc2LoginIdFromNc2UserId($nc3_users, $nc3_photoalbum->update_user_id) . "\"\n";
            ...
            foreach ($nc3_photoalbum_alubums as $nc3_photoalbum_alubum) {
                ...
                $photoalbum_ini .= "created_at                 = \"" . $this->getCCDatetime($nc3_photoalbum_alubum->insert_time) . "\"\n";
                $photoalbum_ini .= "created_name               = \"" . $nc3_photoalbum_alubum->insert_user_name . "\"\n";
                $photoalbum_ini .= "insert_login_id            = \"" . $this->getNc2LoginIdFromNc2UserId($nc3_users, $nc3_photoalbum_alubum->insert_user_id) . "\"\n";
                $photoalbum_ini .= "updated_at                 = \"" . $this->getCCDatetime($nc3_photoalbum_alubum->update_time) . "\"\n";
                $photoalbum_ini .= "updated_name               = \"" . $nc3_photoalbum_alubum->update_user_name . "\"\n";
                $photoalbum_ini .= "update_login_id            = \"" . $this->getNc2LoginIdFromNc2UserId($nc3_users, $nc3_photoalbum_alubum->update_user_id) . "\"\n";
                $photoalbum_ini .= "\n";
            }
            ...
            foreach ($nc3_photoalbum_alubums as $nc3_photoalbum_alubum) {
                ...
                foreach ($nc3_photoalbum_photos as $nc3_photoalbum_photo) {
                    ...
                    $tsv_record['created_at']        = $this->getCCDatetime($nc3_photoalbum_photo->insert_time);
                    $tsv_record['created_name']      = $nc3_photoalbum_photo->insert_user_name;
                    $tsv_record['insert_login_id']   = $this->getNc2LoginIdFromNc2UserId($nc3_users, $nc3_photoalbum_photo->insert_user_id);
                    $tsv_record['updated_at']        = $this->getCCDatetime($nc3_photoalbum_photo->update_time);
                    $tsv_record['updated_name']      = $nc3_photoalbum_photo->update_user_name;
                    $tsv_record['update_login_id']   = $this->getNc2LoginIdFromNc2UserId($nc3_users, $nc3_photoalbum_photo->update_user_id);
            ...
            foreach ($nc3_photoalbum_blocks as $nc3_photoalbum_block) {
                ...
                $slide_ini .= "created_at      = \"" . $this->getCCDatetime($nc3_photoalbum_block->insert_time) . "\"\n";
                $slide_ini .= "created_name    = \"" . $nc3_photoalbum_block->insert_user_name . "\"\n";
                $slide_ini .= "insert_login_id = \"" . $this->getNc2LoginIdFromNc2UserId($nc3_users, $nc3_photoalbum_block->insert_user_id) . "\"\n";
                $slide_ini .= "updated_at      = \"" . $this->getCCDatetime($nc3_photoalbum_block->update_time) . "\"\n";
                $slide_ini .= "updated_name    = \"" . $nc3_photoalbum_block->update_user_name . "\"\n";
                $slide_ini .= "update_login_id = \"" . $this->getNc2LoginIdFromNc2UserId($nc3_users, $nc3_photoalbum_block->update_user_id) . "\"\n";
    ...
    /**
     * NC3：ブロックに紐づくモジュールのメインデータのID 取得
     */
    private function nc3BlockMainDataId($nc3_block)
    {
        ...
        if ($module_name == 'journal') {
            ...
            if (!empty($nc3_journal_block)) {
                $ret = "blog_id = \"" . $this->zeroSuppress($nc3_journal_block->journal_id) . "\"\n";
            }
        } elseif ($module_name == 'bbs') {
            $nc3_bbs_block = Nc2BbsBlock::where('block_id', $nc3_block->block_id)->first();
            // ブロックがあり、掲示板がない場合は対象外
            if (!empty($nc3_bbs_block)) {
                $ret = "blog_id = \"bbs_" . $this->zeroSuppress($nc3_bbs_block->bbs_id) . "\"\n";
            }
        } elseif ($module_name == 'faq') {
            $nc3_faq_block = Nc2FaqBlock::where('block_id', $nc3_block->block_id)->first();
            $ret = "faq_id = \"" . $this->zeroSuppress($nc3_faq_block->faq_id) . "\"\n";
        } elseif ($module_name == 'linklist') {
            $nc3_linklist_block = Nc2LinklistBlock::where('block_id', $nc3_block->block_id)->first();
            // ブロックがあり、リンクリストがない場合は対象外
            if (!empty($nc3_linklist_block)) {
                $ret = "linklist_id = \"" . $this->zeroSuppress($nc3_linklist_block->linklist_id) . "\"\n";
            }
        } elseif ($module_name == 'multidatabase') {
            $nc3_multidatabase_block = Nc2MultidatabaseBlock::where('block_id', $nc3_block->block_id)->first();
            if (empty($nc3_multidatabase_block)) {
                $this->putError(3, "Nc2MultidatabaseBlock not found.", "block_id = " . $nc3_block->block_id, $nc3_block);
            } else {
                $ret = "database_id = \"" . $this->zeroSuppress($nc3_multidatabase_block->multidatabase_id) . "\"\n";
            }
        } elseif ($module_name == 'registration') {
            $nc3_registration_block = Nc2RegistrationBlock::where('block_id', $nc3_block->block_id)->first();
            // ブロックがあり、登録フォームがない場合は対象外
            if (!empty($nc3_registration_block)) {
                $ret = "form_id = \"" . $this->zeroSuppress($nc3_registration_block->registration_id) . "\"\n";
            }
        } elseif ($module_name == 'whatsnew') {
            $nc3_whatsnew_block = Nc2WhatsnewBlock::where('block_id', $nc3_block->block_id)->first();
            $ret = "whatsnew_block_id = \"" . $this->zeroSuppress($nc3_whatsnew_block->block_id) . "\"\n";
        } elseif ($module_name == 'cabinet') {
            $nc3_cabinet_block = Nc2CabinetBlock::where('block_id', $nc3_block->block_id)->first();
            ...
            $nc3_menu_details = Nc2MenuDetail::select('menu_detail.*', 'pages.permalink')
                                             ...
                                             ->where("block_id", $nc3_block->block_id)
                                             ->orderBy('page_id', 'asc')
                                             ->get();
            if (empty($nc3_menu_details)) {
                $ret .= "\n";
                $ret .= "[menu]\n";
                $ret .= "select_flag       = \"0\"\n";
                $ret .= "page_ids          = \"\"\n";
                ...
                foreach ($nc3_menu_details as $nc3_menu_detail) {
                    // 下層ページを含めて取得
                    $ommit_pages = Nc2Page::where('permalink', 'like', $nc3_menu_detail->permalink . '%')->get();
                    if ($ommit_pages->isNotEmpty()) {
                        $ommit_nc3_pages = $ommit_nc3_pages + $ommit_pages->pluck('page_id')->toArray();
                    }
                }
                $ret .= "\n";
                $ret .= "[menu]\n";
            ...
            $nc3_counter = Nc2Counter::where('block_id', $nc3_block->block_id)->first();
            ...
            if (!empty($nc3_calendar_block)) {
                $ret = "calendar_block_id = \"" . $this->zeroSuppress($nc3_calendar_block->block_id) . "\"\n";
            }
        } elseif ($module_name == 'slides') {
            $nc3_slides = Nc2Slides::where('block_id', $nc3_block->block_id)->first();
            // ブロックがあり、スライダーがない場合は対象外
            if (!empty($nc3_slides)) {
                $ret = "slideshows_block_id = \"" . $this->zeroSuppress($nc3_slides->block_id) . "\"\n";
            }
        } elseif ($module_name == 'simplemovie') {
            $nc3_simplemovie = Nc2Simplemovie::where('block_id', $nc3_block->block_id)->first();
            // ブロックがあり、スライダーがない場合は対象外
            if (!empty($nc3_simplemovie)) {
                $ret = "simplemovie_block_id = \"" . $this->zeroSuppress($nc3_simplemovie->block_id) . "\"\n";
            }
        } elseif ($module_name == 'reservation') {
            $nc3_reservation_block = Nc2ReservationBlock::where('block_id', $nc3_block->block_id)->first();
            ...
            if (!empty($nc3_photoalbum_block)) {
                // フォトアルバムでスライド表示は、スライドプラグインに移行
                if ($nc3_photoalbum_block->display == Nc2PhotoalbumBlock::DISPLAY_SLIDESHOW) {
                    $ret = "slideshows_block_id = \"" . $this->zeroSuppress($nc3_photoalbum_block->block_id) . "\"\n";
                } else {
                    $ret = "photoalbum_id = \"" . $this->zeroSuppress($nc3_photoalbum_block->photoalbum_id) . "\"\n";
                }
            }
        }
        return $ret;
    ...
    private function nc3BlockExport($nc3_page, $nc3_block, $new_page_index, $frame_index_str)
    {
        ...
        if ($plugin_name == 'contents') {
            // 固定記事（お知らせ）
            $this->nc3BlockExportContents($nc3_page, $nc3_block, $new_page_index, $frame_index_str);
        } elseif ($plugin_name == 'menus') {
            // メニュー
            // 今のところ、メニューの追加設定はなし。
        } elseif ($plugin_name == 'databases') {
            // データベース
            $this->nc3BlockExportDatabases($nc3_page, $nc3_block, $new_page_index, $frame_index_str);
        } elseif ($plugin_name == 'bbses') {
            // 掲示板
            $this->nc3BlockExportBbses($nc3_page, $nc3_block, $new_page_index, $frame_index_str);
        }
    }
     ...
     */
    private function nc3BlockExportContents($nc3_page, $nc3_block, $new_page_index, $frame_index_str)
    {
        // お知らせモジュールのデータの取得
        // 続きを読むはとりあえず、1つに統合。固定記事の方、対応すること。
        $announcement = Nc2Announcement::where('block_id', $nc3_block->block_id)->firstOrNew([]);
        ...
        if ($announcement->block_id) {
            ...
            $this->putError(1, "no announcement record", "block_id = " . $nc3_block->block_id);
        }
        ...
        $this->nc3Wysiwyg($nc3_block, $save_folder, $content_filename, $ini_filename, $content, 'announcement', $nc3_page);
        $this->nc3Wysiwyg($nc3_block, $save_folder, $content2_filename, $ini_filename, $content2, 'announcement', $nc3_page);
        ...
        $contents_ini .= "insert_login_id = \"" . $this->getNc2LoginIdFromNc2UserId($nc3_users, $announcement->insert_user_id) . "\"\n";
        ...
        $contents_ini .= "update_login_id = \"" . $this->getNc2LoginIdFromNc2UserId($nc3_users, $announcement->update_user_id) . "\"\n";
        $this->storageAppend($save_folder . "/" . $ini_filename, $contents_ini);

        //echo "nc3BlockExportContents";
    }

    /**
     * NC3：汎用データベースのブロック特有部分のエクスポート
     */
    private function nc3BlockExportDatabases($nc3_page, $nc3_block, $new_page_index, $frame_index_str)
    {
        // NC3 ブロック設定の取得
        $nc3_multidatabase_block = Nc2MultidatabaseBlock::where('block_id', $nc3_block->block_id)->first();
        if (empty($nc3_multidatabase_block)) {
     ...
     */
    private function nc3BlockExportBbses($nc3_page, $nc3_block, $new_page_index, $frame_index_str)
    {
        // NC3 ブロック設定の取得
        $nc3_bbs_block = Nc2BbsBlock::where('block_id', $nc3_block->block_id)->first();
        if (empty($nc3_bbs_block)) {
     ...
     * iniファイル名
     */
    private function nc3Wysiwyg($nc3_block, $save_folder, $content_filename, $ini_filename, $content, $nc3_module_name = null, $nc3_page = null)
    {
        // nc3リンク切れチェック
        $nc3_links = $this->getContentHrefOrSrc($content);
        if (is_array($nc3_links)) {
            foreach ($nc3_links as $nc3_link) {
                $this->checkDeadLinkNc2($nc3_link, $nc3_module_name . '(wysiwyg)', $nc3_block);
            }
        }
        ...
        $content = $this->nc3MigrationCommonDownloadMain($nc3_block, $save_folder, $ini_filename, $content, $img_srcs, '[upload_images]');
        ...
        $content = $this->nc3MigrationCommonDownloadMain($nc3_block, $save_folder, $ini_filename, $content, $anchors, '[upload_files]');

        // HTML からa タグの 相対パスリンクを絶対パスに修正
        //$content = $this->changeFullPath($content, $nc3_page);
     ...
     */
    private function nc3MigrationCommonDownloadMain($nc3_block, $save_folder, $ini_filename, $content, $paths, $section_name)
    {
        if (empty($paths)) {
            return $content;
        }
        ...
        // 変換処理
        list($content, $export_paths) = $this->nc3MigrationCommonDownloadMainImple($content, $paths, $section_name, $nc3_block = null);
...
//                            $this->putError(1, "no migrate img", "src = " . $path, $nc3_block);
//                        }
//                    }
//                }
//            }
...
//        }
     ...
     * NC3：common_download_main をエクスポート形式に変換
     */
    private function nc3MigrationCommonDownloadMainImple($content, $paths, $section_name, $nc3_block = null)
    {
        if (empty($paths)) {
            return $content;
        }
    ...
    private function convertNc2Datetime(string $datetime) : ?string
    {
        ...
        // NC3 の投稿日時はGMT のため、9時間プラスする NC3=20151020122600
        $time = mktime(
            substr($datetime, 8, 2),
