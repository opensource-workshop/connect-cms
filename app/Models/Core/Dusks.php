...
class Dusks extends Model
{
     ...
     */
//    public static function getPluginMaterials($dusks, $category, $plugin)
...
/*
                if (property_exists($json_path, 'comment') && !empty($json_path->comment)) {
                    $material['comment'] = self::cleaningText($json_path->comment);
                } else {
                    $material['comment'] = $method->method_title . "です。";
                }
*/
...
//                $json_path->comment = $method->method_title . "です。";
            }
