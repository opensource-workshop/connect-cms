...
@if ($page_obj->isView(Auth::user(), false, true, $page_roles))
    <li class="nav-item">
        ...
        @endfor
            {{$page_obj->page_name}}
            {{-- 子ページの開くアイコン、閉じるアイコン --}}
            @if (count($page_obj->children) > 0 && $page_obj->existChildrenPagesToDisplay($page_obj->children))
    ...
    {{-- 子ページの出力 --}}
    @if (count($page_obj->children) > 0 && isset($page) && ($page_obj->isAncestorOf($page) || $page_obj->id == $page->id))
        @foreach($page_obj->children as $child)
            @include('layouts.opencurrenttree_menu', ['page_obj' => $child])
        @endforeach
    @endif
@endif
