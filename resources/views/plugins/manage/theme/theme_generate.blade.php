...
<script>
    $(function() {
        ...
        $('select.color').change(function() {
            ...
            $(this).css('color',select_val);
        });
        let selected = '';
        let style = '';
        let styles = [];
        $('select.color').each(function(i, e){
            selected = $(e).find('option:selected')[0];
            style = $(selected).attr('style');
            if(style) {
                $.each(style.split(','), function(ii, styles){
                    styles = styles.split(':');
                    $(e).css('color',styles[1]);
                });
            }
        });
    });
    ...
    <div class="card-body">
        <form action="{{url('/')}}/manage/theme/generate" method="POST">
            ...
            <div class="form-group row menu_horizon col-md-4">
                    ...
                    <div class="col-md-8">
                    <select name="menu_horizon_color" id="menu_horizon_color" class="form-control color">
                    @foreach($colors as $color_name => $code)
                        <option value="{{$color_name}}"@if(old('menu_horizon_color', $menu_horizon_color) == $color_name) selected @endif style="color:{{$code}}">■{{$color_name}}</option>
                    @endforeach
                    </select>
                    </div>
                    <label for="menu_horizon_background" class="col-md-4 col-form-label text-md-right">背景色</label>
                    <div class="col-md-8">
                    <select name="menu_horizon_background" id="menu_horizon_background" class="form-control color">
                        <option value="none"@if(old('menu_horizon_background', $menu_horizon_background) == 'none') selected @endif style="color:#000000;">無し</option>
                    @foreach($colors as $color_name => $code)
                        <option value="{{$color_name}}"@if(old('menu_horizon_background', $menu_horizon_background) == $color_name) selected @endif style="color:{{$code}}">■{{$color_name}}</option>
                    @endforeach
                    </select>
                    </div>
                ...
                <div class="form-group row menu_vertical col-md-4">
                    ...
                    <div class="col-md-8">
                    <select name="menu_vertical_color" id="menu_vertical_color" class="form-control color">
                    @foreach($colors as $color_name => $code)
                        <option value="{{$color_name}}"@if(old('menu_vertical_color', $menu_vertical_color) == $color_name) selected @endif style="color:{{$code}}">■{{$color_name}}</option>
                    @endforeach
                    </select>
                    </div>
                    <label for="menu_vertical_background" class="col-md-4 col-form-label text-md-right">背景色</label>
                    <div class="col-md-8">
                    <select name="menu_vertical_background" id="menu_vertical_background" class="form-control color">
                        <option value="none"@if(old('menu_vertical_background', $menu_vertical_background) == 'none') selected @endif style="color:#000000;">無し</option>
                    @foreach($colors as $color_name => $code)
                        <option value="{{$color_name}}"@if(old('menu_vertical_background', $menu_vertical_background) == $color_name) selected @endif style="color:{{$code}}">■{{$color_name}}</option>
                    @endforeach
                    </select>
                    </div>
                ...
                <div class="form-group row frame_tittle col-md-4">
                    ...
                    <div class="col-md-8">
                        <select name="frame_tittle_color" id="frame_tittle_color" class="form-control color">
                        @foreach($colors as $color_name => $code)
                            <option value="{{$color_name}}"@if(old('frame_tittle_color', $frame_tittle_color) == $color_name) selected @endif style="color:{{$code}}">■{{$color_name}}</option>
                        @endforeach
                        </select>
                    </div>
                    <label for="frame_tittle_background" class="col-md-4 col-form-label text-md-right">背景色</label>
                    <div class="col-md-8">
                        <select name="frame_tittle_background" id="frame_tittle_background" class="form-control color">
                            <option value="none"@if(old('frame_tittle_background', $frame_tittle_background) == 'none') selected @endif style="color:#000000;">無し</option>
                        @foreach($colors as $color_name => $code)
                            <option value="{{$color_name}}"@if(old('frame_tittle_background', $frame_tittle_background) == $color_name) selected @endif style="color:{{$code}}">■{{$color_name}}</option>
                        @endforeach
                        </select>
                    </div>
