...
@if (isset($posts))
    @foreach($posts as $post)
        <div class="card mb-3">
            <div class="card-header"><a href="{{url('/')}}/plugin/bbses/show/{{$page->id}}/{{$frame_id}}/{{$post->id}}#frame-{{$frame_id}}">{{$post->title}}</a></div>
            <div class="card-body">
                {!!$post->body!!}
                @if ($children_posts->where("thread_root_id", $post->id)->isNotEmpty())
                    @foreach ($children_posts->where("thread_root_id", $post->id) as $children_post)
                        <div class="card mt-3">
                            <div class="card-header"><a href="{{url('/')}}/plugin/bbses/show/{{$page->id}}/{{$frame_id}}/{{$children_post->id}}#frame-{{$frame_id}}">{{$children_post->title}}</a></div>
                            <div class="card-body">
                                {!!$children_post->body!!}
                            </div>
                        </div>
