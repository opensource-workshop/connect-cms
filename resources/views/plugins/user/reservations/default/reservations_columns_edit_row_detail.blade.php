...
<script type="text/javascript">
    ...
    function submit_update_column_detail() {
        form_selects.action = "{{url('/')}}/plugin/reservations/updateColumnDetail/{{$page->id}}/{{$frame_id}}#frame-{{$frame_id}}";
        form_selects.submit();
    }
...
<form action="" id="form_selects" name="form_selects" method="POST" class="form-horizontal">
    ...
    <div class="alert alert-info mt-2">
        <i class="fas fa-exclamation-circle"></i> {{ $message ? $message : '予約項目【' . $column->column_name . ' 】の詳細設定を行います。' }}
    </div>
