...
<script type="text/javascript">
    function form_delete(id) {
        if (confirm('カテゴリを削除します。\nよろしいですか？')) {
            form_delete_category.action = "{{url('/')}}/plugin/blogs/deleteCategories/{{$page->id}}/{{$frame_id}}/" + id;
            form_delete_category.submit();
        }
    }
</script>

<form action="{{url('/')}}/plugin/blogs/saveCategories/{{$page->id}}/{{$frame_id}}" method="POST" class="">
    {{ csrf_field() }}
