...
<script type="text/javascript">
    function form_delete(id) {
        if (confirm('時間設定を削除します。\nよろしいですか？')) {
            form_delete_pattern.action = "{{url('/')}}/plugin/openingcalendars/deletePatterns/{{$page->id}}/{{$frame_id}}/" + id + "#frame-{{$frame->id}}";
...
<form action="{{url('/')}}/plugin/openingcalendars/savePatterns/{{$page->id}}/{{$frame_id}}/{{$openingcalendar->id}}#frame-{{$frame->id}}" method="POST" class="">
    {{ csrf_field() }}
