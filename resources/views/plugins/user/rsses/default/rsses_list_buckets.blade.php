...
<form action="{{url('/')}}/redirect/plugin/rsses/changeBuckets/{{$page->id}}/{{$frame_id}}#frame-{{$frame->id}}" method="POST" class="">
    {{ csrf_field() }}
            ...
            <thead>
                <tr>
                    ...
                    <th class="text-nowrap">
                        <a href="{{url('/')}}/plugin/rsses/listBuckets/{{$page->id}}/{{$frame_id}}?sort=rsses_updated_at|{{$order_link["rsses_updated_at"][0]}}">更新日</a>
                        @if ($request_order_str == "rsses_updated_at|asc")
                            <i class="fas fa-sort-numeric-down"></i>
                        @elseif ($request_order_str == "rsses_updated_at|desc")
                            <i class="fas fa-sort-numeric-down-alt"></i>
                        @endif
                    </th>
                    {{-- 使用ページ --}}
                    <th class="text-nowrap">
                        <a href="{{url('/')}}/plugin/rsses/listBuckets/{{$page->id}}/{{$frame_id}}?sort=page_name|{{$order_link["page_name"][0]}}">使用ページ</a>
                    ...
                    {{-- フレームタイトル --}}
                    <th class="text-nowrap">
                        <a href="{{url('/')}}/plugin/rsses/listBuckets/{{$page->id}}/{{$frame_id}}?sort=frame_title|{{$order_link["frame_title"][0]}}">フレームタイトル</a>
                    ...
                    {{-- RSSタイトル --}}
                    <th class="text-nowrap">
                        <a href="{{url('/')}}/plugin/rsses/listBuckets/{{$page->id}}/{{$frame_id}}?sort=rsses_name|{{$order_link["rsses_name"][0]}}">RSSタイトル</a>
                        @if ($request_order_str == "rsses_name|asc")
                            <i class="fas fa-sort-alpha-down"></i>
                        @elseif ($request_order_str == "rsses_name|desc")
                            <i class="fas fa-sort-alpha-down-alt"></i>
                        @endif
                    </th>
                </tr>
            ...
            <tbody>
                @foreach($buckets_list as $bucket)
                    <tr @if ($bucket->frames_id == $frame_id) class="cc-active-tr"@endif>
                        ...
                        <td>{{$bucket->rsses_updated_at}}</td>
                        <td>{{$bucket->page_name}}</td>
                        <td>{{$bucket->frame_title}}</td>
                        <td>{{$bucket->rsses_name}}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
