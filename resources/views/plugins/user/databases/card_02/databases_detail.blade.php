{{--
 * 詳細表示画面テンプレート（design-table-dlテンプレートをベース）
 * ・デザインが当てやすいようにclass追加 dl dt ddがつく位置が若干変更されている
 *
...
<div class="container">
    ...
    @foreach($group_rows_cols_columns as $group_row_cols_columns)
        <div class="row border-left border-right border-bottom @if($loop->first) border-top @endif {{ "row-group-" . $row_group_count++ }} database_detail_index_row_{{$loop->index}}">
        {{-- 列グループ ループ --}}
        @foreach($group_row_cols_columns as $group_col_columns)
            <div class="col-sm {{ "column-group-" . $column_group_count++ }} database_detail_index_col_{{$loop->index}}">
            {{-- カラム ループ --}}
                @foreach($group_col_columns as $column)
                    @if ($column_group_count == 2 && $loop->first)
                        {{-- 1列目、且つ、一番最初の項目はタイトル項目とする為、強調表示＆表頭の表示なし --}}
                        <div class="database_detail_col_{{$column->id}}">
                            <h2>
                    ...
                    @else
                        {{-- 上記以外は通常表示 --}}
                        <dl class="database_detail_col_{{$column->id}}">
