...
(function () {
    ...
    var makeItems = function (info) {
      ...
      // add: 画像サイズを表示するか
      var hasImageResize = tinymce.activeEditor.settings.cc_config.has_image_resize;

      // add: アップロード最大サイズ説明
      var uploadMaxFilesizeCaption = {
        name: 'upload_max_filesize_caption',
        type: 'collection',
        label: tinymce.activeEditor.settings.cc_config.upload_max_filesize_caption,
      };
      ...
      var imageResize = {
        ...
        type: 'listbox',
        label: '画像サイズ',
        items: tinymce.activeEditor.settings.cc_config.resized_image_size_items,
      };
      var imageResizeCaption = {
        name: 'image_resize_caption',
        type: 'collection',
        label: '※ 画像アップロード時のみ、画像サイズを指定できます。',
      };
    ...
    var fromImageData = function (image) {
      return {
        ...
        // add: リサイズ画像サイズ、初期値
        resize: tinymce.activeEditor.settings.cc_config.resized_image_size_initial,
      };
    };
      ...
      if (!meta.width && !meta.height && info.hasDimensions) {
        if (isNotEmpty(url)) {
          helpers.imageSize(url).then(function (size) {
            if (state.open) {
              // change: リサイズ可能であれば、画像アップロード時に、幅・高さを自動セットしない（原寸サイズが自動セットされるため、リサイズで小さくしても大きく表示される不都合があるため）
    ...
    var makeDialogBody = function (info) {
      // change: image_uploadtab = false時、タブを表示しないように対応
      // if (info.hasAdvTab || info.hasUploadUrl || info.hasUploadHandler) {
      if (info.hasAdvTab || info.hasUploadTab && (info.hasUploadUrl || info.hasUploadHandler)) {
        var tabPanel = {
          type: 'tabpanel',
          tabs: flatten([
