...
(function () {
    ...
    const makeItems = info => {
      ...
      // add: 画像サイズを表示するか
      const hasImageResize = tinymce.activeEditor.options.get('cc_config').has_image_resize;

      // add: アップロード最大サイズ説明
      const uploadMaxFilesizeCaption = {
      ...
      const imageResize = {
        ...
        type: 'listbox',
        label: '画像サイズ',
        items: tinymce.activeEditor.options.get('cc_config').resized_image_size_items,
      };
      const imageResizeCaption = {
    ...
    const fromImageData = image => ({
      ...
      // add: リサイズ画像サイズ、初期値
      resize: tinymce.activeEditor.options.get('cc_config').resized_image_size_initial,
    });
